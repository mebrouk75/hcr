...
import { QUESTIONS_PATRON, calculatePatronScore, PATRON_DIM_LABELS } from './PatronEngine';
...
const RecruteurTest = () => {
  ...
  const sections = [...new Set(QUESTIONS_PATRON.map(q => PATRON_DIM_LABELS[q.dim]?.label || q.dim))];
  const q = QUESTIONS_PATRON[currentQ];
  const currentSection = PATRON_DIM_LABELS[q?.dim]?.label || q?.dim;
  const sectionIndex = sections.indexOf(currentSection);
  const progress = Math.round(((currentQ) / QUESTIONS_PATRON.length) * 100);

  const handleAnswer = (value) => {
    const newAnswers = { ...answers, [q.id]: value };
    setAnswers(newAnswers);

    if (currentQ < QUESTIONS_PATRON.length - 1) {
      setAnimating(true);
      setTimeout(() => {
      ...
      // Calcul final
      const res = calculatePatronScore(newAnswers);
      setResult(res);
    }
  };
              ...
              {Object.entries(result.dimensions).map(([dim, score]) => (
                <div key={dim}>
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs font-bold text-stone-700 uppercase">{PATRON_DIM_LABELS[dim]?.label || dim}</span>
    ...
    <div className="min-h-screen bg-[#F5F5F0] font-sans flex flex-col">
      ...
      <div className="bg-stone-900 border-b-2 border-stone-700 px-6 py-4 flex items-center justify-between shrink-0">
        ...
        <div className="text-right">
          ...
          <span className="text-white font-black text-lg">{currentQ + 1} / {QUESTIONS_PATRON.length}</span>
        </div>
      </div>
