...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled ? 'bg-[#0A0A0A]/96 backdrop-blur-sm border-b border-[#C9A84C]/15' : 'bg-transparent'
    }`}>
    ...
    <section id="tarifs" className="bg-[#0A0A0A] py-28 px-6 border-t border-stone-900">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {plans.map((plan, i) => (
            <div key={i} className={`relative border-2 p-8 flex flex-col ${
              plan.highlight
              ...
              <button
                ...
                className={`w-full py-3.5 font-black uppercase text-xs tracking-widest transition-all ${
                  plan.highlight
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-5">
            {[
              ...
              {icon:<Mail size={16}/>,title:"contact@lareleve.fr",desc:"Réponse garantie sous 24 heures ouvrées."},
            ].map((item,i) => (
                ...
                <div className="flex gap-2 mb-4">
                  {[{val:'demo',label:'Demander une démo'},{val:'devis',label:'Demander un devis'},{val:'info',label:'Renseignements'}].map(opt => (
                    <button
                      ...
                      className={`flex-1 py-2 text-[10px] font-black uppercase tracking-wide border transition-colors ${
                        form.type === opt.val
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-4 gap-10">
        <div className="md:col-span-2">
          ...
          <div className="flex gap-4 mt-5">
            <a href="mailto:contact@lareleve.fr" className="flex items-center gap-2 text-stone-600 hover:text-[#C9A84C] text-xs font-bold transition-colors">
              <Mail size={12}/>contact@lareleve.fr
            </a>
          </div>
        </div>
        <div>
